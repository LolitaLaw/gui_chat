...
class WindowsTitleBarFix:
    """
    专门用于修复 Windows 11/10 下 Tkinter 窗口标题栏颜色的工具类。
    可以让标题栏变成深色，保持系统圆角。
    """

    @staticmethod
    def apply_dark_title_bar(root):
        """强制应用 Windows 深色标题栏属性"""
        try:
            hwnd = windll.user32.GetParent(root.winfo_id())
            # DWMWA_USE_IMMERSIVE_DARK_MODE = 20 (Windows 11 / Windows 10 2004+)
            value = c_int(1)  # 1 = True (Dark)
            windll.dwmapi.DwmSetWindowAttribute(hwnd, 20, byref(value), sizeof(value))
        except Exception:
            pass  # 非 Windows 系统或版本过低忽略

    @staticmethod
    def apply_light_title_bar(root):
        """强制应用 Windows 浅色标题栏属性"""
        try:
            hwnd = windll.user32.GetParent(root.winfo_id())
            value = c_int(0)  # 0 = False (Light)
            windll.dwmapi.DwmSetWindowAttribute(hwnd, 20, byref(value), sizeof(value))
        except Exception:
            pass
...
class SmartScrollbar(tk.Canvas):
    ...
    def set(self, lo, hi):
        ...
        if not self.auto_hide or self.is_hovering or self.is_dragging:
            self.itemconfig(
                "thumb",
                state="normal",
                fill=self.hover_color if self.is_hovering else self.thumb_color,
            )
        else:
            self.itemconfig("thumb", state="hidden")
    ...
    def on_click(self, e):
        h = self.winfo_height()
        if self.current_lo * h <= e.y <= self.current_hi * h:
            self.is_dragging = True
            self.start_y = e.y
            self.start_lo = self.current_lo
        elif h > 0 and self.command:
            self.command("moveto", e.y / h)
    ...
    def on_enter(self, e):
        ...
        self.itemconfig("thumb", state="normal", fill=self.hover_color)
